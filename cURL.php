...
function get_json($code = 0, $msg, $data = null, $type = 1)
{
	...
	if ($type == 1) {
		return stripslashes(json_encode($josnData, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
	} else {
		return json_encode($josnData, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
	}
}
	...
	public function __call($method, $args)
	{
		switch ($method) {
			...
			case 'header': // 来路
				...
				break;
			case 'proxy':
				$this->options[CURLOPT_PROXY] = "127.0.0.1";
				$this->options[CURLOPT_PROXYPORT]  = "7890";
				break;
			case 'cookie':
				if (isset($args[0])) {
					if (is_array($args[0])) {
